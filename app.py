...
def get_display_name(user_id: str) -> str:
    ...
    try:
        ...
        logger.warning(f"無法e��得用戶暱稱 ({user_id}): {e}")
        return "您"


def send_messages(reply_token: str, messages: list):
    """發送一或多則訊息（TextMessage / FlexMessage 癦可，上限5則）。"""
    with ApiClient(configuration) as api_client:
        line_bot_api = MessagingApi(api_client)
...
def handle_text_message(event):
    ...
    if is_list_request(user_message):
        dining = [c for c in STORE_CARDS if "餐飲" in c["category"]]
        others = [c for c in STORE_CARDS if "餐飲" not in c["category"]]
        msgs = []
        if dining:
    ...
    matched_card = get_matched_store_card(user_message)
...
if __name__ == "__main__":
    ...
    loglogger.info(f"LINE AI 客服啟動，監聽 port {port}")
